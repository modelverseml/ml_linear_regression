...
├── README.md
├── requirements.txt
├── linear_regression.ipynb                          # End-to-end walkthrough on synthetic data
...
The notebook uses **synthetic** regression data generated by `sklearn.datasets.make_regression` — 500 samples, 15 features of which 8 are truly informative and 7 are pure noise. This keeps the repo lightweight (no external file to download) and lets us *check* that the feature-selection step is doing the right thing (it should prune the noise features).

To swap in your own data, replace cell 2 of the notebook with `pd.read_csv(...)` or similar — the rest of the pipeline (scaling, RFE, automated selection, all five model builders) is dataset-agnostic.

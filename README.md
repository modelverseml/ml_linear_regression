...
├── linear_regression/                                   # the package (importable)
│   ├── __init__.py
│   ├── closed_form_model_building.py                    # normal equation: beta = (XᵀX)⁻¹Xᵀy
│   ├── gradient_descent_model_building.py               # batch gradient descent
│   ├── stochastic_gradient_descent_model_building.py    # mini-batch SGD
...
│   ├── automated_feature_selection.py                   # backward elimination (p-value + VIF)
│   ├── recursive_feature_elimination.py                 # RFE wrapper
│   └── variance_inflation_factor_data.py                # VIF (multicollinearity check)
├── examples/
│   └── walkthrough.py                                   # end-to-end demo on synthetic data
└── images/
```
...
```bash
git clone https://github.com/modelverseml/Linear-Regression.git
cd Linear-Regression
pip install -r requirements.txt

# Run the full end-to-end walkthrough (data -> feature selection -> 5 models)
python main.py

# Add residual diagnostic plots at the end
python main.py --plot
```

## Dataset

The walkthrough uses **synthetic** regression data generated by `sklearn.datasets.make_regression`: 500 samples, 15 features of which 8 are truly informative and 7 are pure noise. This keeps the repo lightweight (no external file to download) and lets us *check* that the feature-selection step is doing the right thing (it should prune the noise features).

To swap in your own data, replace `build_dataset()` in [`examples/walkthrough.py`](examples/walkthrough.py) with `pd.read_csv(...)` or similar. The rest of the pipeline (scaling, RFE, automated selection, all five model builders) is dataset-agnostic.
...
All five fitting approaches (closed-form, batch GD, mini-batch SGD, scikit-learn, statsmodels) expose the same `build_model / predict / get_parameters` interface, so they can be swapped freely. Import them from the `linear_regression` package:
...
```python
from linear_regression import RfeClass, final_data, VIF
...
Both methods are implemented from scratch in this repo. See [`closed_form_model_building.py`](linear_regression/closed_form_model_building.py) for the normal-equation solution and [`gradient_descent_model_building.py`](linear_regression/gradient_descent_model_building.py) / [`stochastic_gradient_descent_model_building.py`](linear_regression/stochastic_gradient_descent_model_building.py) for the iterative variants.

These from-scratch implementations are pedagogical; in practice you would use [`sklearn_model_building.py`](linear_regression/sklearn_model_building.py) (scikit-learn's `LinearRegression`) or [`statsmodel_model_building.py`](linear_regression/statsmodel_model_building.py) (statsmodels OLS, which also reports per-coefficient inference statistics).
